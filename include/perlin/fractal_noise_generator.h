...
class linear_decay
{
  ...
  public:
    constexpr T operator()(int i) const noexcept { return s_slope * static_cast<T>(i) + 1; }
};
...
{
  public:
    constexpr T operator()(int i) const noexcept { return 1 / static_cast<T>(i + 1); }
};
...
class exponential_decay
{
  ...
  public:
    constexpr T operator()(int i) const noexcept { return powi(s_base, i); }
};
...
class polynomial_decay
{
  ...
  public:
    constexpr T operator()(int i) const noexcept { return std::pow(i + 1, s_exp); }
};
...
{
  private:
    constexpr static T s_stddev
        = static_powi<2>(static_cast<T>(-Ratio::num) / static_cast<T>(Ratio::den));
 ...
 *
 * @details Fractal noise is generated by summing multiple coherent noise functions at increasing
 * frequencies but with decreasing weight.
 *
 *          Octave frequency increases according to the given frequency function with every step,
 * weight decreases according to the given weighting function. To keep the resulting value within
 * range [-1,1], the summation result is smoothed using a smoothstep function of configurable order.
 *
...
class fractal_noise_generator
{
  public:
    ...
    {
        std::generate(m_weights.begin(),
                      m_weights.end(),
                      [weightFun = WeightFun(), i = 0]() mutable { return weightFun(i++); });
        std::generate(m_frequencies.begin(),
                      m_frequencies.end(),
  ...
  private:
    constexpr point<result_t, dimensions> pointAtOctave(point<result_t, dimensions> p,
